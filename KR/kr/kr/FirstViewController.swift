...
let welcome: [Welcome] = try! decoder.decode([Welcome].self, from: jsonData)
// MARK: - Welcome
...
extension Welcome{
    static func getData() -> [Welcome]{
        return welcome
    }
    
}
class TableViewController: UIViewController {
    let usernames = Welcome.getData()
...
extension TableViewController: UITableViewDataSource, UITableViewDelegate{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return (usernames.first?.accounts.count)!
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        print(123)
        let cell = UITableViewCell(style: .value1, reuseIdentifier: usernames.first?.accounts[indexPath.row].user.name)
        let userName = usernames.first?.accounts[indexPath.row].user.name
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        cell.didSelect(data: usernames.first!.accounts[indexPath.row], at: indexPath)
        self.navigationController?.pushViewController(TableCell(), animated: true)
